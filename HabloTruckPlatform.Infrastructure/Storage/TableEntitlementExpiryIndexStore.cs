...
public sealed class TableEntitlementExpiryIndexStore : IEntitlementExpiryIndexStore
{
        ...
        CancellationToken ct = default)
    {
        ...
        await foreach (var e in _idx.QueryAsync<EntitlementExpiryIndexEntity>(filter: filter, maxPerPage: take, cancellationToken: ct))
        {
            results.Add(new EntitlementExpiryIndexItem(e.CompanyId, e.EntitlementId, e.EndUtc));
